...
async def get_rs(url, session, json={}, md=''):
    ...
    with open('datetime_b.js', 'r', encoding='utf-8') as file:
    ...
    result = subprocess.check_output([r'node', file_name])
    try:
...
async def main():
    async with aiohttp.ClientSession(headers=headers) as session:
        ...
        cookies = await get_rs('http://wapact.189.cn:9000/gateway/stand/detailNew/exchange', session=session, json={}, md='post')
        session.cookie_jar.update_cookies(cookies)
        print(session.cookie_jar.filter_cookies('http://wapact.189.cn:9000'))

        async with session.post('http://wapact.189.cn:9000/gateway/stand/detailNew/exchange', json=json) as res:
